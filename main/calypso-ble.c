...
#define TAG "CALYPSO_BLE"
#define CALYPSO_DATA_SERVICE_UUID  0x180D
#define REMOTE_NOTIFY_CHAR_UUID    0x2A39
...
static esp_bt_uuid_t remote_filter_service_uuid = {
    .len = ESP_UUID_LEN_16,
    .uuid = {.uuid16 = CALYPSO_DATA_SERVICE_UUID,},
};
...
static struct gattc_profile_inst gl_profile_tab[PROFILE_NUM] = {
    [CALYPSO_APP_ID] = {
        .gattc_cb = calypso_profile_event_handler,
...
static void calypso_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_CONNECT_EVT:{
        ...
        gl_profile_tab[CALYPSO_APP_ID].conn_id = p_data->connect.conn_id;
        memcpy(gl_profile_tab[CALYPSO_APP_ID].remote_bda, p_data->connect.remote_bda, sizeof(esp_bd_addr_t));
        ESP_LOGI(TAG, "REMOTE BDA:");
        esp_log_buffer_hex(TAG, gl_profile_tab[CALYPSO_APP_ID].remote_bda, sizeof(esp_bd_addr_t));
    ...
    case ESP_GATTC_SEARCH_RES_EVT: {
        ...
        if (p_data->search_res.srvc_id.uuid.len == ESP_UUID_LEN_16 && p_data->search_res.srvc_id.uuid.uuid.uuid16 == CALYPSO_DATA_SERVICE_UUID) {
            ESP_LOGI(TAG, "service found");
            get_server = true;
            gl_profile_tab[CALYPSO_APP_ID].service_start_handle = p_data->search_res.start_handle;
            gl_profile_tab[CALYPSO_APP_ID].service_end_handle = p_data->search_res.end_handle;
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT:
        ...
        if (get_server){
            ...
            esp_gatt_status_t status = esp_ble_gattc_get_attr_count( gattc_if,
                                                                     ...
                                                                     gl_profile_tab[CALYPSO_APP_ID].service_start_handle,
                                                                     gl_profile_tab[CALYPSO_APP_ID].service_end_handle,
                                                                     INVALID_HANDLE,
                                                                     &count);
            ...
            if (count > 0){
                ...
                if (!char_elem_result){
                    ...
                    if (count > 0 && (char_elem_result[0].properties & ESP_GATT_CHAR_PROP_BIT_NOTIFY)){
                        gl_profile_tab[CALYPSO_APP_ID].char_handle = char_elem_result[0].char_handle;
                        esp_ble_gattc_register_for_notify (gattc_if, gl_profile_tab[CALYPSO_APP_ID].remote_bda, char_elem_result[0].char_handle);
                    }
                }
                /* free char_elem_result */
                free(char_elem_result);
    ...
    case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
        ...
        if (p_data->reg_for_notify.status != ESP_GATT_OK){
            ...
            esp_gatt_status_t ret_status = esp_ble_gattc_get_attr_count( gattc_if,
                                                                         gl_profile_tab[CALYPSO_APP_ID].conn_id,
                                                                         ESP_GATT_DB_DESCRIPTOR,
                                                                         gl_profile_tab[CALYPSO_APP_ID].service_start_handle,
                                                                         gl_profile_tab[CALYPSO_APP_ID].service_end_handle,
                                                                         gl_profile_tab[CALYPSO_APP_ID].char_handle,
                                                                         &count);
    ...
    case ESP_GATTC_NOTIFY_EVT:
        ...
        if (p_data->notify.value_len == 10){
            // Wind Update Received
            uint8_t wind_data[10];
            memcpy(wind_data, p_data->notify.value, p_data->notify.value_len);
            ...
            if (adv_name != NULL) {
                if (strlen(remote_device_name) == adv_name_len && strncmp((char *)adv_name, remote_device_name, adv_name_len) == 0) {
                    ...
                    if (connect == false) {
                        ...
                        esp_ble_gattc_open(gl_profile_tab[CALYPSO_APP_ID].gattc_if, scan_result->scan_rst.bda, scan_result->scan_rst.ble_addr_type, true);
                    }
                }
            }
            break;
...
void app_main(void)
{
    ...
    ret = esp_ble_gattc_app_register(CALYPSO_APP_ID);
    if (ret){
